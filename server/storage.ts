...
export interface IStorage {
  ...
  enrollPatient(enrollment: PatientEnrollment): Promise<Patient>;
  ...
  // Data Export methods
  createDataExport(exportData: InsertDataExport): Promise<DataExport>;
  getDataExports(userId?: number): Promise<DataExport[]>;
  ...
  createUserAssessment(data: InsertUserAssessment): Promise<UserAssessment>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getPatients(clinicianId?: number): Promise<PatientWithDetails[]> {
    ...
    if (clinicianId) {
      baseQuery.where(eq(patients.assignedClinicianId, clinicianId));
    }

    const results = await baseQuery;
    
    // Transform results to PatientWithDetails format
    const patientsWithDetails = await Promise.all(
      results.map(async (row) => {
        const patient = row.patients;
        const cohort = row.cohorts;
        const assignedClinician = row.clinical_users;
  ...
  async getPatientWithDetails(id: number): Promise<PatientWithDetails | undefined> {
    ...
    return {
      ...
      assignedClinician: result.clinical_users,
      lastAssessment: lastAssessment || null,
  ...
  async getCohortAssessments(cohortId: number, limit?: number): Promise<PatientAssessment[]> {
    ...
    return results.map(row => row.patient_assessments);
  }
  ...
  async createInjuryType(injuryType: InsertInjuryType): Promise<InjuryType> {
    const [newType] = await db.insert(injuryTypes).values(injuryType).returning();
    return newType;
  }
